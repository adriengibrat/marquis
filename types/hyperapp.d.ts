...
//
type ActionPayload<DPayload, CPayload> = CPayload extends void
  ? void
  : (CPayload | ((data: DPayload) => CPayload));
 ...
 * @memberOf [App]
 */
export type Dispatch<State, AcceptPayload> = (
  obj: Dispatchable<State, any, any>,
  data: AcceptPayload
) => void;
 ...
 * @memberOf [App]
 */
export type EffectFunc<State, Props, NextPayload, Result = void> = (
  dispatch: Dispatch<State, NextPayload>,
  props: Props,
  payload: any
) => Result;
 ...
 * @memberOf [App]
 */
export type Effect<State, Props, NextPayload = void> = [
  EffectFunc<State, Props, NextPayload>,
  Props
];
 ...
 * @memberOf [App]
 */
export type PureAction<State, Payload = void> = Payload extends void
  ? (state: State) => State
 ...
 * @memberOf [App]
 */
export type ActionOnInit<State, Payload = void> = Payload extends void
 ...
 * @memberOf [App]
 */
export type Subscription<State, Props, Payload = void> = [
  EffectFunc<State, Props, Payload, () => void>,
  Props
];
 ...
 * @memberOf [App]
 */
export type SubscriptionsResult<State> =
  | (Subscription<State, any> | boolean)[]
 ...
 * @memberOf [App]
 */
export function a<S, DPayload>(
  action: Action<S, DPayload>
): Dispatchable<S, DPayload, void>;
 ...
 * @memberOf [App]
 */
export function a<S, DPayload, PPayload>(
  action: Action<S, PPayload>,
  payload: ActionPayload<DPayload, PPayload>
): Dispatchable<S, DPayload, PPayload>;

export namespace hyperapp {
  export function app<State>(app: AppProps<State>): void;
}
...
type JSXAttribute = Partial<
  { [key in EventKeys]: Dispatchable<any, EventParameterType<key>, any> }
> & { [key: string]: any };
